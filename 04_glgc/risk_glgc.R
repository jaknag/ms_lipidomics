...
outcome_dat = format_data(ms_dat %>% 
                            dplyr::select(-SNP) %>% 
                            dplyr::rename("SNP"=rsID,"pval"=P),type="outcome")
...
for(file in gwas_raw_files){
  ...
  fharm_name = paste0(glgc_path,"risk/working/",exposure_name,"_harmonise",".csv")
  fres_name = paste0(glgc_path, "risk/results/",exposure_name,"_results",".csv")
  fressingle_name = paste0(glgc_path, "risk/results/",exposure_name,"_resultssingle",".csv")
  fresmrpresso_name = paste0(glgc_path, "risk/results/",exposure_name,"_mrpresso",".csv")
  combinedresults = paste0(glgc_path, "risk/results/","_combined",".csv")
