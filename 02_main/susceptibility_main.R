...
OUTCOME_TSV  <- "/Users/nagrodzkij/cam/IMT/MS/lipidomics/extended/risk/outcomes/ms_risk.tsv"
EXPO_DIR     <- "/Users/nagrodzkij/cam/IMT/MS/lipidomics/extended/exposures"
WORK_DIR     <- "/Users/nagrodzkij/cam/IMT/MS/lipidomics/extended/risk/working/new"
RESULTS_DIR  <- "/Users/nagrodzkij/cam/IMT/MS/lipidomics/extended/risk/results/new"
...
# Read MS susceptibility GWAS; rename to TwoSampleMR conventions; create chr:pos "SNP"

ms_dat = readr::read_tsv(
  OUTCOME_TSV,
  col_types = "dcccdddddddc"
  ) %>% 
...
for (file in gwas_raw_files) {
  ...
  # Collect results
  singleresults$pval.MR_PRESSO_global <- as.character(singleresults$pval.MR_PRESSO_global)
